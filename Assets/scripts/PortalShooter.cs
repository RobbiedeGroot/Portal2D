...
public class PortalShooter : MonoBehaviour
{
	...
	public KeyCode keyToPress = KeyCode.Mouse0;
	public KeyCode keyToPress2 = KeyCode.Mouse1;
	...
	public bool OrangePortalEnable;
	public bool BluePortalEnable;
	...
	void Update()
	{
		if (Input.GetKey(keyToPress) && Time.time >= timeOfLastSpawn + creationRate && OrangePortalEnable == true)
		{
			Vector2 actualBulletDirection = (relativeToRotation) ? (Vector2)(Quaternion.Euler(0, 0, transform.eulerAngles.z + 45) * shootDirection) : shootDirection;
			
			//portalAudio.Play();
			GameObject newObject = Instantiate<GameObject>(prefabToSpawn);
			newObject.GetComponent<SpriteRenderer>().color = new Color(1f, 0.5f, 0f, 1f);
		...
		if (Input.GetKey(keyToPress2) && Time.time >= timeOfLastSpawn + creationRate && BluePortalEnable == true )
		{
			Vector2 actualBulletDirection = (relativeToRotation) ? (Vector2)(Quaternion.Euler(0, 0, transform.eulerAngles.z + 45) * shootDirection) : shootDirection;
			
			//portalAudio.Play();
			GameObject newObject = Instantiate<GameObject>(prefabToSpawn);
			newObject.GetComponent<SpriteRenderer>().color = new Color(0f, 1f, 1f, 1f);
			newObject.transform.position = this.transform.position;
			newObject.transform.eulerAngles = new Vector3(0f, 0f, Utils.Angle(actualBulletDirection));
	...
	private void OnCollisionEnter2D(Collision2D other) {
		...
		if(other.gameObject.CompareTag("Blue")){
			BluePortalEnable = true;
			Debug.Log("blue");
			Destroy(other.gameObject);
		}

		if(other.gameObject.CompareTag("Orange")){
			OrangePortalEnable = true;
			Destroy(other.gameObject);
		}
	}
}
